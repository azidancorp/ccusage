...
#[cfg(test)]
use crate::fast::FxHashSet;
use crate::{CodexTokenUsageEvent, Result, chunk_file_indexes_by_size};

#[cfg(test)]
use super::paths::CodexUsageSource;
use super::{
...
pub(crate) fn load_codex_events_from_directory(
    ...
    let mut events = load_codex_events_for_files(sessions_dir, &files, single_thread);
...
fn load_codex_events_from_sources(
    ...
    for group in collect_deduped_codex_usage_files(sources) {
        let mut source_events =
            load_codex_events_for_files(&group.dir, &group.files, single_thread);
...
pub(crate) fn source_files() -> Result<Vec<PathBuf>> {
    ...
    let mut files = if let [source] = sources.as_slice() {
        collect_codex_usage_files(&source.dir)
    } else {
        collect_deduped_codex_usage_files(&sources)
            .into_iter()
...
pub(super) fn load_codex_events_for_files(
    sessions_dir: &Path,
    files: &[PathBuf],
    single_thread: bool,
...
mod tests {
    ...
    #[test]
    fn counts_forked_from_id_token_history_for_personal_totals() {
        let fixture = fs_fixture!({
            "2026-05-12T08-00-00-parent.jsonl": [
                json!({
                    "timestamp": "2026-05-12T08:00:00.000Z",
        ...
        for single_thread in [true, false] {
            ...
            assert_eq!(
                events.len(),
                5,
                "expected 5 events (2 parent + 3 fork token_count lines), got {} with single_thread={}",
                events.len(),
                single_thread
            );
            ...
            assert_eq!(fork_events[0].input_tokens, 1_000);
            assert_eq!(fork_events[0].cached_input_tokens, 100);
            assert_eq!(fork_events[0].output_tokens, 200);
            assert_eq!(fork_events[0].total_tokens, 1_200);
            assert_eq!(fork_events[1].input_tokens, 500);
            assert_eq!(fork_events[1].cached_input_tokens, 50);
            assert_eq!(fork_events[1].output_tokens, 100);
            assert_eq!(fork_events[1].total_tokens, 600);
            assert_eq!(fork_events[2].input_tokens, 100);
            assert_eq!(fork_events[2].cached_input_tokens, 10);
            assert_eq!(fork_events[2].output_tokens, 20);
            assert_eq!(fork_events[2].total_tokens, 120);
        }
    }
