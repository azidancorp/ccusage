...
mod tests {
    ...
    #[test]
    fn applies_long_context_rates_with_auto_speed_windows_per_event() {
        let mut pricing = PricingMap::default();
        pricing.load_json(
            r#"{
                "gpt-5.5": {
        ...
        let events = ["2026-05-08T23:59:59.999Z", "2026-05-09T00:00:00.000Z"]
            .into_iter()
            .map(|timestamp| CodexTokenUsageEvent {
                session_id: "session-1".to_string(),
                timestamp: timestamp.to_string(),
        ...
        assert_eq!(
            report["monthly"][0]["costUSD"],
            serde_json::json!(2_700_000)
        );
        assert_eq!(report["totals"]["costUSD"], serde_json::json!(2_700_000));
    }
