...
The original **“cc”** came from **C**laude **C**ode usage and now also fits **C**odex **C**LI usage. As OpenCode, Amp, Droid, Codebuff, Hermes Agent, pi-agent, Goose, OpenClaw, Kilo, Kimi, Qwen, Gemini CLI, Antigravity, and other coding (agent) CLIs became part of the same workflow, ccusage expanded into a general name for local coding CLI usage analysis.
...
The `personal` branch additionally supports Antigravity:

| Agent       | ID            | Default data location                                |
| ----------- | ------------- | ---------------------------------------------------- |
| Antigravity | `antigravity` | `${ANTIGRAVITY_DATA_DIR:-~/.gemini/antigravity-cli}` |
...
Antigravity support on this branch is heuristic because Antigravity does not expose official local token counts. See [Source Support Q&A](/guide/source-support-qa) for caveats and for current notes on Grok CLI and Devin CLI.
